...
def output_acc(model, replay_buffer, test_buffer, bgimage):
    ...
    def calc_acc(buffer, is_train):
        obs, actions, rewards, nextobs, _, env_id, poss, poss_next = buffer.sample(
            batch_size)
...
def main():
    ...
    for i in range(train_envs+test_envs):
        try:
            ...
            map_config = imp.load_source(
                'map_config', map_config_file).map_config
   ...
   # ob = ob [20:100,20:100,:]
    pos = np.array(
        envlist[env_index].env.env.p.game.newGame.Players[0]._Person__position)
    ...
    for i in range(train_envs+test_envs):
       ...
       # ob = ob [20:100,20:100,:]
        ...
        for j in range(fore_max_iter):
            ...
            tmp[j] = ob
        if i == 0:
            bgimage = np.empty([train_envs+test_envs] +
                               list(ob.shape), dtype=np.uint8)
        tmp, _ = mode(tmp)
    ...
    for i in range(test_batch_num):
        test_obs, test_actions, test_rewards, test_nextob, _, test_env_id, test_pos, test_nextpos = test_buffer.sample(
            batch_size)
    ...
    for i in range(max_iter):
       ...
       # ob = ob [20:100,20:100,:]
        ...
        if (replay_buffer.can_sample(batch_size) and i > learn_start):
            obs, actions, rewards, nextobs, _, env_id, poss, poss_next = replay_buffer.sample(
                batch_size)
